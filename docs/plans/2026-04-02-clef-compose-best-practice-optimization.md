...
| Commit | Phase | 描述 |
|--------|-------|------|
| `47596af` | P1 | add maxTurns and Edit/Grep tools to composer, harmonist, rhythmist, orchestrator |
| `3494110` | P1 | add maxTurns and Edit/Grep tools to reviewer, revision, leader |
| `e32f047` | P2 | split theory.md into 6 role-specific sub-skills |
| `22f4608` | P2 | preload theory sub-skills in composer, harmonist, rhythmist |
| `c27c18a` | P2 | preload theory sub-skills in revision, orchestrator, reviewer, leader |
| `9d66615` | P3+P4 | add auto-validation hook and agent data contract to SKILL.md |
| `4d6f246` | P4 | add project memory to reviewer and composer |

### 审核修复 Commit

| Commit | 严重度 | 描述 |
|--------|--------|------|
| `e73b874` | HIGH | replace stale theory.md references with sub-skill names in all agents |
| `68f382b` | MEDIUM | tighten hook regex and fix cross-references in sub-skills |
| `1ccd108` | MEDIUM | add theory-orchestration to composer and reviewer skills（解决交叉引用悬空） |
| `2cde5f4` | MEDIUM | sync theory-abc drum map with abc_to_midi.py DRUM_MAP |

### 审核结果

- **Round 1**: 2 HIGH + 4 MEDIUM → 全部修复
- **Round 2**: 1 MEDIUM（交叉引用悬空）→ 已修复
- **鼓映射表**: theory-abc 的鼓映射表与 abc_to_midi.py DRUM_MAP 不一致（F,/c/d/A, 对应错误，缺少 e/Mid Tom 和 G,/Open HH），已与权威源同步

### 与原计划的偏差

1. **Composer/Reviewer 额外预加载 theory-orchestration** — 原计划中 Composer 只预加载 theory-melody + theory-abc，Reviewer 只预加载 theory-structure + theory-abc。审核发现两者在 agent body 中引用了 theory-orchestration 的内容（乐器演奏约束、配器平衡原则），因此额外添加了预加载。
2. **Hook regex 收紧** — 原计划用 `Write.*score\.abc`，审核建议改为 `Write.*\.clef-work/score\.abc` 避免误触发。
3. **鼓映射表修复** — 原计划未预见此问题，在审核过程中发现并修复。
