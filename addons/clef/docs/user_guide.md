...
钢琴卷帘位于传输控制栏和混音台之间，实时显示 MIDI 音符在时间轴上的分布。
...
钢琴卷帘计划升级为可操作的审查微调工具，支持：

- 音符编辑（选中、移动、删除、改音高/力度）
- 撤销/重做（Ctrl+Z / Ctrl+Shift+Z）
- 播放标注（标记问题音符，生成结构化 Agent 反馈）
- 临时音符与屏蔽（试听效果）
- 导出修改后的 MIDI / ABC 文件

详细设计参见 [piano-roll-interactive-design.md](../../docs/plans/2026-04-02-piano-roll-interactive-design.md)。
