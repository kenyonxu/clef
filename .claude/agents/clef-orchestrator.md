...
**原则：相邻 energy_level 间 CC7 峰值差距不超过 15。** 过大的差距会导致段间音量跳变突兀。

| balance_intent | melody | harmony | bass | 说明 |
|----------------|--------|---------|------|------|
| `melody_forward` | 90-100 | 78-88 | 82-90 | 旋律穿透，伴奏退后 |
| `epic_tutti` | 98-108 | 88-98 | 90-100 | 全员推进，低频加厚 |
| `intimate` | 82-90 | 68-78 | 72-80 | 透明薄织体 |
| `rhythmic_drive` | 80-90 | 75-85 | 95-105 | 节奏驱动 |
| （空/缺省） | 90-98 | 78-88 | 82-90 | 默认平衡 |
...
相邻段落间的 CC7 过渡**不可瞬间跳变**，必须用渐变（ramp）衔接：

| energy 差距 | 最短渐变时长 | 示例 |
|------------|-------------|------|
| ≤2 | **4 拍**（~3秒） | energy 3→5：4 拍从 CC7=85 渐变到 95 |
| ≥3 | **8 拍**（~6秒） | energy 3→7：8 拍从 CC7=85 渐变到 108 |

渐变应在目标段落开始**之前**触发（即过渡跨越段落边界）。渐变方向：
- 低→高：线性或指数增长
- 高→低：线性或指数衰减
...
- **工具提供客观数据，你做创意判断**
- CC7 在段落级别设定基准值，段内应有 **±3-5 的微弱起伏**（每 4-8 拍），模拟自然演奏的呼吸感。起伏幅度不超过基准值的 ±8%
- CC11 用于乐句内微动态
- 鼓声部（Ch9）不设 CC7
...
```
Section A [intimate], 8 小节 (32 拍), melody CC7 基准 85:
  beat  0: CC7=85  (段落开始)
  beat  6: CC7=88  (+3, 乐句高点)
  beat 12: CC7=82  (-3, 乐句低点)
  beat 18: CC7=87  (+2, 再次高点)
  beat 24: CC7=83  (-2, 再次低点)
  beat 30: CC7=88  (过渡渐变开始，向 A2 渐强)
  beat 32: CC7=95  (A2 基准值，4 拍过渡)
```

## CC 策略

> **inject_expression.py 基线感知**：当源 MIDI 无 CC7/CC11 事件时，注入脚本会自动：
> - CC7：缩放峰值到 100（播放器默认）
> - CC11：**跳过注入**（保持播放器默认 127）
>
> 因此，你应设计**相对动态范围**而非绝对值。CC7 峰值不需精确对齐 100，脚本会自动缩放。关键在于段间差距和段内起伏的比例。源 MIDI 无 CC11 时，你生成的 CC11 事件会被丢弃，应将动态变化全部放在 CC7 上。

| CC | 用途 | 典型用法 |
|----|------|----------|
| CC7 | 通道音量 | **段级基准 + 段内呼吸起伏**（主控制器，承载所有动态变化） |
| CC11 | 表现力 | **仅乐句内微动态**（幅度 ±10-15，每段 3-5 个事件），不用于段级变化 |
| CC1 | 颤音 | 长音或高音位置添加颤音 |
...
- CC7 段级基准 + 段内 ±3-5 呼吸起伏，CC11 仅乐句内微动态（±10-15）
