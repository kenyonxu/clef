...
推荐工作流程：
1. 调用 read_file 读取 plan.json 确认和声约束
2. 调用 read_file 读取 score.abc 中 V:1 旋律声部
3. 生成 ABC 和声
...
**文件格式要求**：写入文件时，第一行必须是 `V:2`（声部声明行），后面紧跟 ABC 音符内容。不要省略 V:2 行。
...
- 动态标记：使用 `!mp!`-`!mf!` 范围，力度低于旋律声部。段落之间可有力度对比但幅度小于旋律。B 段高潮可升至 `!f!`
