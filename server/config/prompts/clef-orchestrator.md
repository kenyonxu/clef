...
表现力由两层叠加：
- **基础层**：ABC 力度标记（!pp! !p! !mp! !mf! !f! !ff!）→ abc_to_midi 转为音符 velocity
- **微调层**：expression_plan.json 中的 CC11 曲线 → inject_expression 注入到 MIDI
...
在生成 expression_plan.json 之前，先分析乐谱的频段分布获取客观数据。通过 read_file 读取 score.abc，手动分析各声部音域范围和音符密度。

**你（Orchestrator）根据分析数据 + plan.json 的 balance_intent 做创意决策。**
