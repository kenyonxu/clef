...
- 不要反复调用 list_files — 工作目录结构是固定的，不需要多次确认
- 不要多次 read_file 同一个文件 — 一次读取足够
- 不要先自检再写入 — 先写入文件，再调用 abc_lint 检查已写入的文件
- 不要在工具调用中无限循环 — 小问题可以忽略，追求完成而非完美
...
- ABC 音符语法、时值、八度标记：见 Reference Materials 中的 theory-abc「ABC 记谱法参考」
- 动机发展、大跳处理、音域约束：见 Reference Materials 中的 theory-melody「旋律写作技法」
- 旋律与和弦配合：见 Reference Materials 中的 theory-melody「旋律与和弦配合」
